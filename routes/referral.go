...
func registerReferrals(router *httptreemux.TreeMux) {
	...
	router.PUT("/referral_codes/:code", impl.validate)
}
...
func (impl *referralsImpl) create(w http.ResponseWriter, r *http.Request, params map[string]string) {
	referrals, err := middlewares.CurrentUser(r).CreateReferrals(r.Context())
	if err != nil {
		views.RenderErrorResponse(w, r, err)
	} else {
		views.RenderReferralCodes(w, r, referrals)
...
func (impl *referralsImpl) validate(w http.ResponseWriter, r *http.Request, params map[string]string) {
